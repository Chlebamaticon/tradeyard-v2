...
describe('Order', function () {
  ...
  describe('Deployment', function () {
    it('Should OrderStatus be "Created"', async function () {
      const { order } = await loadFixture(deployOrderFixture);

      expect(await order.asOwner.read.getOrderStatus()).to.equal(0);
    });

    it('Should set the right owner', async function () {
      const { order, wallets } = await loadFixture(deployOrderFixture);
  ...
  describe('Transitions', function () {
    describe('From "Created" to "MerchantRelease"', () => {
      let fixture;
      beforeEach(async () => {
        fixture = fixture ? fixture : await loadFixture(deployOrderFixture);
      });
    ...
    describe('From "Created" through "CustomerComplaint" to "ModeratorComplaintReleased"', () => {
      ...
      it('Should transition to "CustomerDeposit"', async () => {
        ...
        expect(await getBalances()).to.deep.equal({
          customer: 0n,
          merchant: 0n,
          owner: 0n,
          escrow: tokenAmount,
